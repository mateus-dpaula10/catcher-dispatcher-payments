...
class PaidSusanPetRescueController extends Controller
{
    ...
    public function paid(Request $request)
    {
        ...
        if (!$isPaidByWebhook && $statusFromWebhookRaw === 'processing' && $eventId === 'donation_status_changed') {
            Log::info('GiveWP status=processing tratado como paid para completar fluxo', [
                'event'       => $eventId ?: null,
                'status'      => $statusFromWebhookRaw,
                'donation_id' => $donationIdFromWebhook ?: null,
                'email'       => $whEmail ?: null,
            ]);
            $isPaidByWebhook = true;
        }
        ...
        elseif (
            str_contains($methodLower, 'stripe') ||
            str_contains($methodLower, 'credit_card') ||
            str_contains($methodLower, 'card') ||
            str_contains($methodLower, 'apple_pay') ||
            str_contains($methodLower, 'google_pay')
